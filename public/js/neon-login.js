	...
	$(document).ready(function() {
		...
		$.extend(neonLogin, {
			...
			resetProgressBar : function(display_errors) {
				TweenMax.set(neonLogin.$container, {
					css : {
						opacity : 0
					}
				});
		...
		if (is_lockscreen) {
			...
			var bg = neonLogin.lockscreen_progress_canvas, ctx = ctx = bg.getContext('2d'), imd = null, circ = Math.PI * 2, quart = Math.PI / 2, currentProgress = 0;
